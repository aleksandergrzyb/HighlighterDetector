...
void AGCornerDetector::findCornersInImage(Mat& image, vector<Point>& imageCorners)
{
    ...
    if (!contours.empty()) {
        ...
        dilate(drawing, drawing, element, Point(-1,-1), 1);
        erode(drawing, drawing, element, Point(-1,-1), 1);
        
        // Second procedure of finding contours and creating convex hull around contours.
        cvtColor(drawing, drawing, CV_BGR2GRAY);
        findContours(drawing, contoursTwo, hierarchyTwo, CV_RETR_CCOMP, CV_CHAIN_APPROX_NONE, Point(0, 0));
        vector<vector<Point>> hullTwo(contoursTwo.size());
...
void AGCornerDetector::sortCorners(vector<Point>& corners)
{
    ...
    Point topLeftPoint = corners[topLeftPointIndex], topRightPoint = corners[topRightPointIndex], bottomRightPoint = corners[bottomRightPointIndex], bottomLeftPoint = corners[bottomLeftPointIndex];
    corners.clear();
...
void AGCornerDetector::cropDetectedCornersInImage(Mat& image, vector<Point>& corners, int distance)
{
    this->sortCorners(corners);
    vector<double> lineCoefficientVector;
    vector<Point> pointsVector;
    Point newPoint, middlePointNew, movePointNew, middlePoint;
